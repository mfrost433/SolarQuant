...
if((args.startDate == None) & (args.endDate == None)):
    pr.updateDatum(nodeId, srcId)
    pr.addPredictionInput(nodeId, srcId)
else:
    start = dt.datetime.strptime(args.startDate,"%Y-%m-%d:%H")
    end = dt.datetime.strptime(args.endDate, "%Y-%m-%d:%H")

    pr.updateDatum(nodeId, srcId, end, start)
    pr.addPredictionInput(nodeId, srcId,args.endDate, args.startDate)
